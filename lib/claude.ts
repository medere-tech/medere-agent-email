...
export interface GenerateMailParams {
  ...
  formationsLiees: Array<{ nom: string; format: string }>
}
...
export async function generateMail(params: GenerateMailParams): Promise<{ sujet: string; corps: string }> {
  ...
  const upsellText =
    formationsLiees.length > 0
      ? `\n\nJ'en profite également pour vous partager d'autres formations susceptibles de vous intéresser :\n${formationsLiees.map((f) => `• ${f.nom} (${f.format})`).join('\n')}`
      : ''
